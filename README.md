...
NEXUS keeps all your work in named **Workspaces** — persistent containers stored in a local SQLite database. Each workspace remembers its Data Tables and reports across sessions.

- **Workspace switcher** in the sidebar — create, rename, switch, or delete workspaces
- **Data Tables panel** — lists every saved table in the active workspace; click to open in CSV Editor, delete to remove
- **Save to workspace** — available in CSV Editor, SQL Runner (Snapshot), and Report Builder
- **Activity timeline** on the home screen — last 6 actions (saved tables, generated reports) with timestamps
- **Ctrl+K / ⌘K command palette** — search across all workspace Data Tables and all 30 tools from any screen; keyboard-navigable with ↑↓ and Enter
...
| SQL Runner     | A full SQL workbench running entirely offline. **Local mode** loads CSV, JSON, or XLSX files into an in-browser SQLite engine (sql.js WASM); workspace datasets are automatically loaded as queryable tables (◆ badge) so you can JOIN across files without re-importing. **Server mode** connects to live PostgreSQL, MySQL, and MSSQL databases via native Node.js drivers. Multi-tab query editor with syntax highlighting, live autocomplete, and Ctrl+Enter shortcuts. **Results:** sort, filter, copy, column stats, paginated view (200 rows), export as CSV / JSON / SQL INSERT, and a **Snapshot** button that saves any result set directly to the active workspace as a Data Table. **Query history:** persistent, 30-day rolling window, grouped by Today / Yesterday / This week / by calendar week — click any entry to restore it. **Schema sidebar** with lazy-loaded indexes, database switcher, and search. **Toolbar:** Format SQL, Visual EXPLAIN tree, ER Diagram (draggable SVG with FK arrows), Pivot table, Chart mode (8 ECharts chart types). Connection health chip shows live ping latency. |
| Report Builder | Compose structured reports from a split editor/preview layout. **Section types:** Heading, Text, Table, and Chart. **7 themes:** Classic, Navy, Forest, Rose, Slate, Ember, and Midnight — each with gradient banner, styled headings, themed table headers, stat cards, and footer accent stripe. **6 chart types** via ECharts with user-controlled X/Y axes and sort/limit options. Export to PDF via native save dialog. Save reports to the active workspace for later retrieval. Reached from CSV Editor ("Generate Report") or directly via Ctrl+K. |
