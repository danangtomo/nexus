...
[![Version](https://img.shields.io/badge/version-0.15.0-007AFF?style=flat-square)](https://github.com/danangtomo/nexus/releases)
...
_Productivity tools. Zero internet. Everything local._
...
NEXUS is a free, open-source desktop app that bundles all productivity tools into one clean interface. No subscriptions, no cloud uploads, no telemetry — everything runs on your machine.
...
| Rich Text Editor   | A Notion-style document editor powered by TipTap. Stores multiple named documents locally in SQLite with debounced auto-save and a save indicator. **Toolbar:** undo/redo; font family (System, Serif, Mono, Cursive) and size (Small/Normal/Large/Huge) selectors; paragraph and heading styles (H1–H3); bold, italic, underline, strikethrough, inline code, subscript, superscript; multi-color highlight (7 swatches + clear); font color picker; link bar (inline URL input — no `window.prompt`); left/center/right/justify alignment; bullet list, numbered list, task list (interactive checkboxes with strikethrough on completion), blockquote, code block, horizontal rule; image insertion (file dialog → stored as base64 so images survive app restarts and navigation); table insert with full row/column management (add/delete rows and columns, delete table). **Find bar** (Ctrl+F): search with match count, Next/Previous navigation. **Export:** DOCX with real embedded images (OOXML `wp:inline` drawing with EMU-scaled dimensions), PDF (Electron printToPDF with correctly sized margins), and plain TXT. **Import:** DOCX via mammoth.js. Print button with `@media print` chrome suppression. Word and character count in the status bar.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| Tool               | What it does    |
| ------------------ | --------------- |
| Password Generator | _(coming soon)_ |
| File Encryptor     | _(coming soon)_ |
| Hash Generator     | _(coming soon)_ |
| Unit Converter     | _(coming soon)_ |
| Color Converter    | _(coming soon)_ |
| Base64 Encoder     | _(coming soon)_ |
| Regex Tester       | _(coming soon)_ |
...
For a complete list of bundled components and their licenses, see the list below.

Full npm dependency license details are available in [licenses.json](licenses.json) (auto-generated via `npx license-checker --production --json`).
